...
class QuantumWordleSolver:
    ...
    def get_next_guess(self) -> str:
        ...
        # Convert words to numeric representation for quantum processing
        word_nums = self._words_to_numbers(self.possible_words)
    ...
    def _words_to_numbers(self, words: Set[str]) -> List[int]:
        """Convert words to binary numbers for quantum processing"""
        return [sum(1 << i for i, c in enumerate(word)) for word in words]
...
class PositionEvaluationCircuit:
    ...
    def build(self) -> QuantumCircuit:
        ...
        # Multi-controlled Z gate using MCXGate
        mcx_gate = MCXGate(self.qubit_count)
        qc.h(self.qubit_count)
        qc.append(mcx_gate, list(range(self.qubit_count + 1)))
        qc.h(self.qubit_count)
